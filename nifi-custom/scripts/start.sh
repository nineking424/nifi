...
## Additional configurable NiFi properties #
# Core Properties #
prop_replace 'nifi.flow.configuration.file'         "${NIFI_FLOW_CONFIGURATION_FILE:-./conf/flow.xml.gz}"
prop_replace 'nifi.flow.configuration.json.file'    "${NIFI_FLOW_CONFIGURATION_FILE:-./conf/flow.json.gz}"
prop_replace 'nifi.flow.configuration.archive.dir'  "${NIFI_FLOW_CONFIGURATION_FILE:-./conf/archive/}"
# Content Repository
prop_replace 'nifi.content.repository.archive.max.usage.percentage'  "${NIFI_CONTENT_REPOSITORY_ARCHIVE_MAX_USAGE_PERCENTAGE:-50%}"

## Default configurable NiFi properties #1
